...
action: 'click' | 'credentials' | 'report' | 'video_completed' | 'quiz_pass' | 'quiz_fail' | 'game_played' | 'game_high_score'
): number {
...
action === 'report'            ? +10  :
    action === 'click'             ? -20  :
    action === 'credentials'       ? -40  :
    action === 'video_completed'   ? +10  :
    action === 'quiz_pass'         ? +20  :
    action === 'quiz_fail'         ? +5   :
    action === 'game_played'       ? +5   :
    action === 'game_high_score'   ? +15  : 0;
...
export async function getCampaignIdsForCompany(
  ...
  return campaigns.map((c: any) => c._id as mongoose.Types.ObjectId);
}
...
export async function computeUserAnalytics(userId: string, companyId?: string) {
  ...
  if (user.companyId) {
    ...
    rank = higher + 1;
    percentile = totalInCo > 0 && rank !== null
