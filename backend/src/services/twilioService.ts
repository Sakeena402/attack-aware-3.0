...
export const generateTrackingUrl = (
  ...
  // Must point to BACKEND port 5000 — this is what goes in the SMS
  const baseUrl = process.env.TRACKING_BASE_URL || 'http://localhost:5000/api/track';
  const params = new URLSearchParams({ t: token, c: campaignId, u: userId, p: pageType });
  return `${baseUrl}/click?${params.toString()}`;
};
...
export const makeVoiceCall = async (options: MakeCallOptions): Promise<{
  ...
  // ── MOCK MODE ──────────────────────────────────────────────────────────────
  if (process.env.MOCK_SMS === 'true') {
    const mockSid = `MOCK_CA_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
