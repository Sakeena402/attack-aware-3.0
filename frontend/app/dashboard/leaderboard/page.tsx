...
import { api, LeaderboardEntry, Employee } from '@/app/services/api';
import { Trophy, Medal, Award, TrendingUp, TrendingDown, Minus, Crown, Star, Users } from 'lucide-react';
...
export default function LeaderboardPage() {
  ...
  const { data: leaderboard, isLoading } = useSWR<LeaderboardEntry[]>(
    state.user?.companyId 
      ? `/leaderboard?companyId=${state.user.companyId}${filterDepartment !== 'all' ? `&department=${filterDepartment}` : ''}`
      : `/leaderboard${filterDepartment !== 'all' ? `?department=${filterDepartment}` : ''}`,
    fetcher,
    ...
    <div className="space-y-8">
      ...
      {!isLoading && top3.length >= 3 && (
        ...
        >
          ...
          >
            <div className="relative">
              <div className="w-18 h-18 rounded-full bg-gradient-to-br from-orange-500 to-orange-600 flex items-center justify-center text-white font-bold text-xl mb-3 shadow-lg" style={{ width: '72px', height: '72px' }}>
