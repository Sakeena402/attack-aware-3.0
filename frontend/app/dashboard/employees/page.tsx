...
import { api, employeeApi, Employee } from '@/app/services/api';
...
export default function EmployeesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Refresh data
      mutate(state.user?.companyId ? `/employees?companyId=${state.user.companyId}` : '/employees');
      closeModal();
    } catch (err) {
    ...
    <div className="space-y-8">
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage employees and track their training progress
          </p>
        </div>
          ...
          <>
            ...
            <Card className="p-4 bg-gradient-to-br from-green-500/10 to-transparent border-green-500/20">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-foreground">{stats.trained}</p>
      ...
      >
        ...
        <select
          ...
          className="px-4 py-2 bg-muted/50 border border-purple-500/20 rounded-lg text-sm text-foreground focus:outline-none focus:border-purple-500/50 focus:ring-2 focus:ring-purple-500/20 transition-all"
        >
                ...
                >
                  <Card className={`p-6 border-2 ${riskConfig.border} bg-gradient-to-br from-muted/30 to-transparent overflow-hidden relative group hover:shadow-lg hover:shadow-purple-500/10 transition-all duration-300`}>
                    ...
                    <div className="absolute top-4 right-4">
                      <motion.div
                        className={`p-2 rounded-lg ${riskConfig.bg} ${riskConfig.text}`}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-2">
              Password {selectedEmployee && <span className="text-muted-foreground font-normal">(leave blank to keep current)</span>}
            </label>
            <input
              type="password"
              ...
              className="w-full px-4 py-2 bg-muted/50 border border-purple-500/20 rounded-lg text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-purple-500/50 focus:ring-2 focus:ring-purple-500/20"
              placeholder={selectedEmployee ? '********' : 'Create password'}
              minLength={6}
            />
          </div>
